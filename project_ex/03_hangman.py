...
def get_random_word(word_list):
    """
    맞출 대상이 되는 문자열들의 리스트에서 임이의 단어 하나를 선택하여 반환한다.
...
def get_guess(already_guessed):
    ...
    while True:
        ...
        elif user_guess in already_guessed:                 # already_guessed list 에 user_guess 가 있는지 확인
...
while True:
    ...
    if len(missed_letters) == len(HANGMAN_PICS) - 1:    # 행맨의 목숨이 남아 있는지 확인
        display_board()
